...
public abstract class ExecuteAsRootBase {
	...
	public final boolean executeCommandList() {
		boolean retval = false;
	...
	public final DataInputStream executeSingleCommand(String commands) {
		boolean retval = false;
			...
			if (null != commands && commands.length() > 0) {
				...
				try {
					...
					if (255 != suProcessRetval) {
						// Root access granted
						retval = true;
						return new DataInputStream(suProcess.getInputStream());
					} else {
						// Root access denied
						retval = false;
						return null;
